...
This same principle also reflects the broader logic of the course project. The assignment explicitly prioritizes methodological rigor and asks students to explain why a specific approach was chosen over alternatives, rather than presenting broad and superficial experimentation. The regularization project proposal likewise defines the study as a controlled comparison in which the training pipeline is fixed and only the regularization method changes, so that the effects on overfitting, generalization, and training stability can be observed clearly and fairly. Because the course itself introduces regularization and optimization as structured components of deep learning methodology, this one-factor-at-a-time principle provided the most appropriate foundation for the entire project.
...
This project fits the course expectations because it focuses on a **single methodological theme** and studies it in depth rather than attempting to cover a wide range of topics superficially. The central aim is not to maximize benchmark performance, but to examine how different regularization strategies affect model behavior under controlled conditions. This makes the project well suited to a course that emphasizes understanding training methodology, model behavior, and experimental reasoning.
...
This transition was also consistent with the methodological structure of the project proposal. The regularization study was originally defined as a controlled comparison of multiple methods, but the expected output was not only a “best method” label; rather, it was a results section showing **which method is most suitable under which condition**. A full-combo stage therefore became important because it allowed the study to move beyond one-factor-at-a-time diagnosis and toward the construction of a stronger integrated training setup informed by the earlier findings.
...
This subsection is especially important because the instructor explicitly expects the project to explain **why a specific approach was chosen over alternatives**.
...
Regularization remained the main topic because it was the most coherent way to connect the project to the course emphasis on overfitting, generalization, dropout, L1/L2, early stopping, and batch normalization.  It also matched the original project proposal, which defined the work as a controlled regularization comparison and only suggested optimizer comparison as a secondary extension. Keeping regularization as the main axis therefore preserved both the methodological focus and the course alignment of the project.
...
This project is closely connected to the methodological framework of the course because it studies deep learning not only as a predictive task, but as a problem of **model behavior under controlled training conditions**. Rather than treating overfitting, regularization, and optimization as separate themes, the project brings them together within a single experimental setting and examines how they interact. In this sense, the study reflects the course approach by focusing on methodology, comparison, and interpretation rather than on benchmark performance alone.

A central point of this connection is the treatment of **overfitting as the main analytical problem**. The project begins with an intentionally overfitting baseline and evaluates model behavior through training, validation, and test performance, together with generalization gaps, instead of relying only on final accuracy. This makes the analysis consistent with a course methodology that emphasizes understanding learning behavior rather than only reporting end results.

The same methodological connection appears in the regularization study itself. L1/L2 penalties, dropout, early stopping, and batch normalization are not presented as isolated techniques, but are examined under shared experimental conditions so that their effects can be compared directly. This reflects a course-oriented way of studying deep learning methods: not only asking whether a method works, but also asking **how it works, what kind of problem it addresses, and under which conditions it becomes useful**.

The optimization section extends this logic further. After establishing a stronger regularized configuration, the project examines optimizer, learning-rate, and scheduler choices as factors that shape convergence and final model quality. As a result, the report follows a methodological progression that is fully consistent with the structure of the course: first diagnosing overfitting, then studying regularization systematically, and finally refining the training process through optimization.
